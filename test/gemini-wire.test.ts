...
describe('Gemini Wire Protocol Tests', () => {
  ...
  const geminiSystemRequestFixture = JSON.parse(
    fs.readFileSync(path.join(__dirname, 'fixtures/gemini-system-request.json'), 'utf8')
  );
  
  const geminiSystemResponseFixture = fs.readFileSync(
    path.join(__dirname, 'fixtures/gemini-system-response.json'), 'utf8'
  );
  
  const geminiRequestFixture = JSON.parse(
    fs.readFileSync(path.join(__dirname, 'fixtures/gemini-request.json'), 'utf8')
  );
  
  const geminiResponseFixture = fs.readFileSync(
    path.join(__dirname, 'fixtures/gemini-response.json'), 'utf8'
  );
  ...
  it('should correctly handle Gemini response with schema', async () => {
    ...
    if (typeof result === 'string') {
      ...
      const cleanResult = result.includes('```') 
        ? result.replace(/```json\n|\n```|```\n|\n```/g, '')
        : result;
  ...
  it('should correctly handle Gemini response with system message', async () => {
    ...
    if (typeof result === 'string') {
      ...
      const jsonMatch = (result as string).match(/```json\s*([\s\S]*?)\s*```/) || 
                        (result as string).match(/```\s*([\s\S]*?)\s*```/) || 
                        [null, result as string];
