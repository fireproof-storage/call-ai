...
describe('Simple callAI integration tests', () => {
  ...
  describe('Non-structured text generation', () => {
    ...
    modelEntries.map(([modelName, modelId]) => {
      ...
      gradeAwareTest(modelId)(`should generate text with ${modelName} model without streaming`, async () => {
      ...
      // Test with streaming
      gradeAwareTest(modelId)(`should generate text with ${modelName} model with streaming`, async () => {
      ...
      itif(!!haveApiKey)(`should generate structured data with ${modelName} model using schema`, async () => {
        ...
        try {
          ...
          if (data.name && data.capital && data.population) {
            ...
            expectOrWarn(modelId, typeof data.population === 'number', `'population' is not a number in ${modelName} model response`);
            expectOrWarn(modelId, data.name.toLowerCase().includes('france'), `'name' should include 'france' in ${modelName} model response`);
            expectOrWarn(modelId, data.capital.toLowerCase().includes('paris'), `'capital' should include 'paris' in ${modelName} model response`);
          }
        } catch (e) {
  ...
  describe('Complex schema validation', () => {
    ...
    // Run all model tests concurrently
    modelEntries.map(([modelName, modelId]) => {
      itif(!!haveApiKey)(`should generate and validate complex structured data with ${modelName} model`, async () => {
        ...
        try {
          ...
          if (data.destination) expectOrWarn(modelId, typeof data.destination === 'string', `'destination' is not a string in ${modelName} model response`);
          if (data.duration) expectOrWarn(modelId, typeof data.duration === 'number', `'duration' is not a number in ${modelName} model response`);
          if (data.budget) expectOrWarn(modelId, typeof data.budget === 'number', `'budget' is not a number in ${modelName} model response`);
          if (data.activities) expectOrWarn(modelId, Array.isArray(data.activities), `'activities' is not an array in ${modelName} model response`);
          if (data.accommodation) expectOrWarn(modelId, typeof data.accommodation === 'object', `'accommodation' is not an object in ${modelName} model response`);
          if (data.transportation) expectOrWarn(modelId, typeof data.transportation === 'object', `'transportation' is not an object in ${modelName} model response`);
          ...
          if (data.accommodation) {
            ...
            expectOrWarn(modelId, !!data.accommodation.features, `Missing 'accommodation.features' in ${modelName} model response`);
            expectOrWarn(modelId, !!data.accommodation.price, `Missing 'accommodation.price' in ${modelName} model response`);
            
            if (data.accommodation.name) expectOrWarn(modelId, typeof data.accommodation.name === 'string', `'accommodation.name' is not a string in ${modelName} model response`);
            if (data.accommodation.type) expectOrWarn(modelId, typeof data.accommodation.type === 'string', `'accommodation.type' is not a string in ${modelName} model response`);
            if (data.accommodation.features) expectOrWarn(modelId, Array.isArray(data.accommodation.features), `'accommodation.features' is not an array in ${modelName} model response`);
            if (data.accommodation.price) expectOrWarn(modelId, typeof data.accommodation.price === 'number', `'accommodation.price' is not a number in ${modelName} model response`);
          ...
          if (data.transportation) {
            ...
            if (data.transportation.mode) expectOrWarn(modelId, typeof data.transportation.mode === 'string', `'transportation.mode' is not a string in ${modelName} model response`);
            if (data.transportation.cost) expectOrWarn(modelId, typeof data.transportation.cost === 'number', `'transportation.cost' is not a number in ${modelName} model response`);
          }
          
          // Value range validation
          if (data.duration) expectOrWarn(modelId, data.duration > 0, `'duration' is not positive in ${modelName} model response`);
          if (data.budget) expectOrWarn(modelId, data.budget > 0, `'budget' is not positive in ${modelName} model response`);
          if (data.accommodation?.price) expectOrWarn(modelId, data.accommodation.price > 0, `'accommodation.price' is not positive in ${modelName} model response`);
          if (data.transportation?.cost) expectOrWarn(modelId, data.transportation.cost > 0, `'transportation.cost' is not positive in ${modelName} model response`);
        } catch (e) {
          expectOrWarn(modelId, false, `JSON parse error in ${modelName} model response: ${e}`);
        }
      }, TIMEOUT);
  ...
  describe('Complex flat schema validation', () => {
    ...
    // Run all model tests concurrently
    modelEntries.map(([modelName, modelId]) => {
      itif(!!haveApiKey)(`should generate data with complex flat schema using ${modelName} model`, async () => {
        // Make the API call with schema
        const result = await callAI(
          'Create a recipe for a healthy dinner.',
          {
            apiKey: process.env.CALLAI_API_KEY,
            model: modelId.id,
        ...
        try {
          ...
          // Check types
          if (data.name) expectOrWarn(modelId, typeof data.name === 'string', `'name' is not a string in ${modelName} model response`);
          if (data.cuisine) expectOrWarn(modelId, typeof data.cuisine === 'string', `'cuisine' is not a string in ${modelName} model response`);
          if (data.prepTime) expectOrWarn(modelId, typeof data.prepTime === 'number', `'prepTime' is not a number in ${modelName} model response`);
          if (data.cookTime) expectOrWarn(modelId, typeof data.cookTime === 'number', `'cookTime' is not a number in ${modelName} model response`);
          if (data.servings) expectOrWarn(modelId, typeof data.servings === 'number', `'servings' is not a number in ${modelName} model response`);
          if (data.difficulty) expectOrWarn(modelId, typeof data.difficulty === 'string', `'difficulty' is not a string in ${modelName} model response`);
          if (data.ingredients) expectOrWarn(modelId, Array.isArray(data.ingredients), `'ingredients' is not an array in ${modelName} model response`);
          if (data.steps) expectOrWarn(modelId, Array.isArray(data.steps), `'steps' is not an array in ${modelName} model response`);
          
          // Check arrays
          if (Array.isArray(data.ingredients)) expectOrWarn(modelId, data.ingredients.length > 0, `'ingredients' array is empty in ${modelName} model response`);
          if (Array.isArray(data.steps)) expectOrWarn(modelId, data.steps.length > 0, `'steps' array is empty in ${modelName} model response`);
          
          // Check data values
          if (data.prepTime) expectOrWarn(modelId, data.prepTime > 0, `'prepTime' is not positive in ${modelName} model response`);
          if (data.cookTime) expectOrWarn(modelId, data.cookTime > 0, `'cookTime' is not positive in ${modelName} model response`);
          if (data.servings) expectOrWarn(modelId, data.servings > 0, `'servings' is not positive in ${modelName} model response`);
        } catch (e) {
          expectOrWarn(modelId, false, `JSON parse error in ${modelName} model response: ${e}`);
        }
      }, TIMEOUT);
  ...
  describe('Simple nested schema validation', () => {
    ...
    // Run all model tests concurrently
    modelEntries.map(([modelName, modelId]) => {
      itif(!!haveApiKey)(`should generate data with simple nested schema using ${modelName} model`, async () => {
        // Make the API call with schema
        const result = await callAI(
        ...
        try {
          ...
          if (data.root && typeof data.root === 'object') {
            ...
            if (data.root.name) expectOrWarn(modelId, typeof data.root.name === 'string', `'root.name' is not a string in ${modelName} model response`);
            if (data.root.type) expectOrWarn(modelId, typeof data.root.type === 'string', `'root.type' is not a string in ${modelName} model response`);
            if (data.root.children) {
              expectOrWarn(modelId, Array.isArray(data.root.children), `'root.children' is not an array in ${modelName} model response`);
              expectOrWarn(modelId, data.root.children.length > 0, `'root.children' array is empty in ${modelName} model response`);
              ...
              if (Array.isArray(data.root.children) && data.root.children.length > 0) {
                ...
                if (firstChild) {
                  ...
                  if (firstChild.name) expectOrWarn(modelId, typeof firstChild.name === 'string', `'firstChild.name' is not a string in ${modelName} model response`);
                  if (firstChild.type) expectOrWarn(modelId, typeof firstChild.type === 'string', `'firstChild.type' is not a string in ${modelName} model response`);
                  if (firstChild.children) expectOrWarn(modelId, Array.isArray(firstChild.children), `'firstChild.children' is not an array in ${modelName} model response`);
                  ...
                  if (Array.isArray(firstChild.children) && firstChild.children.length > 0) {
                    ...
                    if (secondChild) {
                      ...
                      if (secondChild.name) expectOrWarn(modelId, typeof secondChild.name === 'string', `'secondChild.name' is not a string in ${modelName} model response`);
                      if (secondChild.type) expectOrWarn(modelId, typeof secondChild.type === 'string', `'secondChild.type' is not a string in ${modelName} model response`);
                    }
                  }
                }
              }
