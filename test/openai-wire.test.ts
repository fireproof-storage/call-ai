...
describe('OpenAI Wire Protocol Tests', () => {
  ...
  const openaiResponseFixture = fs.readFileSync(
    path.join(__dirname, 'fixtures/openai-response.json'), 'utf8'
  );
  
  const openaiStreamRequestFixture = JSON.parse(
    fs.readFileSync(path.join(__dirname, 'fixtures/openai-stream-request.json'), 'utf8')
  );
  
  const openaiStreamResponseFixture = fs.readFileSync(
    path.join(__dirname, 'fixtures/openai-stream-response.json'), 'utf8'
  );
  ...
  it('should correctly format OpenAI JSON schema request', async () => {
    ...
    expect(actualRequestBody.response_format.json_schema.name)
      .toEqual(openaiRequestFixture.response_format.json_schema.name);
    
    expect(actualRequestBody.response_format.json_schema.schema.properties)
      .toEqual(openaiRequestFixture.response_format.json_schema.schema.properties);
  });
