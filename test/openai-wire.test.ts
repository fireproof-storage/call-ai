...
describe("OpenAI Wire Protocol Tests", () => {
  ...
  const openaiRequestFixture = JSON.parse(
    fs.readFileSync(
      path.join(__dirname, "fixtures/openai-request.json"),
      "utf8",
    ),
  );
  ...
  it("should correctly format OpenAI JSON schema request", async () => {
    ...
    expect(actualRequestBody.response_format.type).toEqual(
      openaiRequestFixture.response_format.type,
    );
    ...
    expect(
      actualRequestBody.response_format.json_schema.schema.properties,
    ).toEqual(
  ...
  it("should use JSON schema format for GPT-4o with schema handling", async () => {
    ...
    expect(actualRequestBody.response_format.json_schema.name).toBe(
      "book_recommendation",
    );
    expect(actualRequestBody.response_format.json_schema.schema).toBeTruthy();
    expect(
      actualRequestBody.response_format.json_schema.schema.properties.title,
    ).toBeTruthy();
  ...
  it("should support tool mode for OpenAI models when enabled", async () => {
    ...
    if (actualRequestBody.tools) {
      ...
      expect(
        actualRequestBody.tools[0].input_schema.properties.title,
      ).toBeTruthy();
